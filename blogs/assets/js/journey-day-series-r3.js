...
function applyJourneySeo(entry, basePath) {
    const safeBasePath = String(basePath || "/blogs/journey").replace(/\/$/, "");
...
async function renderEntryAsync(entry, index, total, items) {
    ...
    applyJourneySeo(entry, basePath);
...
function renderMissing() {
    ...
    setCanonical(`${SITE_URL}${basePath.replace(/\/$/, "")}/day/`);
